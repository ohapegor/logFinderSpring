...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="searchInfoResult">
        ...
        <xsl:value-of select="searchInfo/regexp"/><xsl:text>\cell\row\trowd\trleft5000\cellx7500\cellx10000Location\cell</xsl:text>
        <xsl:value-of select="searchInfo/location"/><xsl:text>\cell\row</xsl:text>
        ...
        <xsl:for-each select="searchInfo/dateIntervals">
            <xsl:text>\trowd\trleft5000\cellx7500\cellx10000 </xsl:text><xsl:value-of select="dateFrom"/><xsl:text>\cell </xsl:text>
            <xsl:value-of select="dateTo"/><xsl:text>\cell\row</xsl:text>
        </xsl:for-each>
        ...
        <xsl:text>\par{\pard\ansi\ansicpg1251\deff0\adeflang1049 \qc </xsl:text><xsl:value-of select="emptyResultMessage"/><xsl:text>  \par } </xsl:text>
        </xsl:if>
        <xsl:apply-templates select="resultLogs"/>
        <xsl:text> } </xsl:text>
    </xsl:template>

    <xsl:template match="resultLogs">
        ...
        <xsl:text>\b File Name : \b0 </xsl:text><xsl:value-of select="fileName"/><xsl:text> \line </xsl:text>
        <xsl:text>\b Message : \b0 </xsl:text><xsl:value-of select="content"/><xsl:text>\line \par } </xsl:text>
    </xsl:template>
